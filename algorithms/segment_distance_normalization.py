...
assert len(edges) == 2*(NUM_JOINTS-1)
...
assert len(skeleton_edges) == (NUM_JOINTS - 1)
...
def accumulate_distance(frame, u, v, avg_dist):
  if not (u>=0 and u<NUM_JOINTS and v>=0 and v<NUM_JOINTS):
    return
...
def accumulate_frame_distances(frame, avg_dist):
  assert frame.shape == (NUM_JOINTS, 3)
  start = ROOT_JOINT_INDEX
...
def apply_segment_distance_normalization(sequence):
  if not APPLY_SEGMENT_DISTANCE_NORMALIZATION:
    raise ValueError
...
def check_normalization(sequence):
  ...
  for frame in sequence:
    ...
    for (u, v) in skeleton_edges:
      ...
      assert abs(get_distance(frame[u], frame[v]) - avg_dist[u][v])<=TOLERANCE, f"Distance did not match {get_distance(frame[u], frame[v])} and {avg_dist[u][v]}"